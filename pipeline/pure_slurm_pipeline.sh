...
commonargs=" -A ls_polle --parsable -n 1 -J $video_id"
...
# use --gpus=1 --gres=gpumem:33g or --gpus=1 --gres=gpumem:41g instead of --gpus=rtx_3090:1 if memory is not enough
# memory: 414 + n MB (200% redundancy)
# sdfstudio_train_args="--cpus-per-task=16 --mem-per-cpu=$((414 + ${num_images}))M --gpus=1 --gres=gpumem:41g"
# sdfstudio_train_args="--cpus-per-task=16 --mem-per-cpu=$((414 + ${num_images}))M --gpus=1 --gres=gpumem:33g"
sdfstudio_train_args="--cpus-per-task=16 --mem-per-cpu=$((414 + ${num_images}))M --gpus=rtx_3090:1"
...
# sdfstudio render time, conservative estimation: (1500 + 12*n_image) /60 minutes, or 25 + n/5 minutes
# memory: 281 + n/2 MiB of mem (200% redundancy)
...
sbatch $commonargs $post_processing_args -d afterok:$sdfstudio_render_flag,afterok:$sdfstudio_extract_flag --time=$post_processing_time --wrap="rm -rf ${target_dir}/intermediate/sdfstudio_preprocessing"

monitor_jobids=($download_preprocessing_flag $gsam_1_flag $gsam_2_flag $ovseg_1_flag $ovseg_2_flag $intern_1_flag $intern_2_flag $mask3d_1_flag $mask3d_2_flag $omnidata_flag $hha_flag $cmx_1_flag $cmx_2_flag $consensus_flag $point_lifting_flag $sdfstudio_train_flag $sdfstudio_extract_flag $sdfstudio_render_flag)

monitor_jobids=($download_preprocessing_flag $gsam_1_flag $gsam_2_flag $ovseg_1_flag $ovseg_2_flag $intern_1_flag $intern_2_flag $mask3d_1_flag $mask3d_2_flag $omnidata_flag $hha_flag $cmx_1_flag $cmx_2_flag $consensus_flag $point_lifting_flag)

# after rendering, change the group of folder, get statistics
sbatch $commonargs $post_processing_args -d singleton --output=$log_dir/${video_id}/final.out --time=$post_processing_time --wrap="export SACCT_FORMAT=\"JobID,JobName,User,Partition,NodeList,Elapsed,State,ExitCode,MaxRSS,AveRSS,AllocTRES,AveCPU,AveCPUFreq,CPUTime\" && for jobid in $(echo ${monitor_jobids[@]}); do (sacct -j \$jobid && seff \$jobid) > $log_dir/${video_id}/stats_\${jobid}.out; done && chgrp -R INFK-CVG-Euler-All ${target_dir}"
