...
def convert_poses(scene_dir):
    ...
    poses = np.loadtxt(scene_dir / 'traj_w_c.txt',
    ...
    intrinsics = np.array([[320, 0, 320, 0], [0, 320, 240, 0], [0, 0, 1, 0],
                           [0, 0, 0, 1]])
    (scene_dir / 'intrinsic').mkdir(exist_ok=True)
    np.savetxt(scene_dir / 'intrinsic' / 'intrinsic_color.txt', intrinsics)
...
if __name__ == '__main__':
    ...
    log.info(
        'converting poses from semantic-nerf replica format to scannet format')
