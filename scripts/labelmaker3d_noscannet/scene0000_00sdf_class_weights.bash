...
scene=scene0000_00
experiment_name=${scene}_noscannet_class_weights
echo $scene
echo $scene

mkdir $TMPDIR/$scene
for SUBDIR in color depth label-filt intrinsic omnidata_depth omnidata_normal pose pred_sam pred_consensus pred_consensus_noscannet refinedpose pred_consensus_5_scannet
do
    echo Copying $SUBDIR ...
	cp -r /cluster/project/cvg/blumh/scannet/$scene/$SUBDIR $TMPDIR/$scene/
done

python scripts/sdfstudio_scannet_preprocessing.py --label_template pred_consensus_noscannet --sampling 2 --size 416 --scannetpose \
    $TMPDIR/$scene
