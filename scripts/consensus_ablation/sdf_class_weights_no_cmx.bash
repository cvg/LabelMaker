...
mkdir $TMPDIR/$scene
for SUBDIR in pred_consensus_5_scannet_no_cmx color depth label-filt intrinsic omnidata_depth omnidata_normal pose pred_sam pred_consensus refinedpose pred_consensus_5_scannet
do
    echo Copying $SUBDIR ...
	cp -r /cluster/project/cvg/blumh/scannet/$scene/$SUBDIR $TMPDIR/$scene/
done

python scripts/sdfstudio_scannet_preprocessing.py --label_template pred_consensus_5_scannet_no_cmx --sampling 2 --size 416 --scannetpose \
    $TMPDIR/$scene
