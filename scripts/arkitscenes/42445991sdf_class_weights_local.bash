...
scene='42445991'
experiment_name=${scene}_class_weights_local
echo $scene
echo $scene

scenefolder=/home/weders/scratch/scratch/scannetter/arkit/raw/Validation/$scene

python scripts/sdfstudio_arkitscenes_preprocessing.py --label_template pred_consensus_noscannet --sampling 2 --size 416 --scannetpose $scenefolder
sleep 5
...
ns-train neus-facto \
    ...
    --output-dir $scenefolder/output \
    --vis wandb \
    sdfstudio-data \
    --data $scenefolder/sdfstudio \
    --include-sensor-depth True \
    --include-semantics True \
    --include-mono-prior True

outfolder=$scenefolder/output/$(ls $scenefolder/output)/neus-facto
...
ns-render --camera-path-filename $scenefolder/sdfstudio/camera_path.json \
    --traj filename \
    --output-format images \
    --rendered-output-names semantics \
    --output-path $scenefolder/pred_sdfstudio_$train_id.png \
    --load-config $config
