...
def _dist_get_unmatched_confmat(scene_dir, keys, pred_space, label_space,
  ...
  left_id_to_confmat_idx = np.zeros(max(matcher.left_ids) + 1, dtype=np.int64)
  for i, left_id in enumerate(matcher.left_ids):
    left_id_to_confmat_idx[left_id] = i + 1
  right_id_to_confmat_idx = np.zeros(max(matcher.right_ids) + 1, dtype=np.int64)
  for i, right_id in enumerate(matcher.right_ids):
    right_id_to_confmat_idx[right_id] = i + 1
  for k in tqdm(keys):
    pred = cv2.imread(
        str(scene_dir / pred_template.format(k=(k // subsampling))),
        cv2.IMREAD_UNCHANGED)
    label = cv2.imread(str(scene_dir / label_template.format(k=k)),
                       cv2.IMREAD_UNCHANGED)
    if pred.shape[0] != label.shape[0] or pred.shape[1] != label.shape[1]:
...
def evaluate_scene(scene_dir,
                   ...
                   pred_template_glob='pred/{k}.png',
                   label_template='label_filt/{k}.png',
                   label_template_glob='label_filt/{k}.png',
                   overwrite_confmat=False,
                   n_jobs=8):
  scene_dir = Path(scene_dir)
  if keys is None:

    files = glob(str(scene_dir / label_template_glob.format(k='*')),
                 recursive=True)
    keys = sorted(
        int(re.search(label_template_glob.format(k='(\d+)'), x).group(1))
        for x in files)
...
def evaluate_scenes(scene_dirs,
  ...
  for k, scene_dir in enumerate(scene_dirs):
    ...
    _, c = evaluate_scene(scene_dir,
                          pred_space,
                          label_space,
                          pred_template=pred_template[k]
                          if type(pred_template) is list else pred_template,
                          pred_template_glob=pred_template_glob,
                          label_template=label_template,
                          label_template_glob=label_template_glob,
                          subsampling=subsampling,
                          overwrite_confmat=overwrite_confmat,
                          n_jobs=n_jobs)
    if confmat is None:
