...
python scripts/sdfstudio_replica_preprocessing.py --replica True --sampling 4 \
    /media/blumh/data/replica/$replicaroom/Sequence_1/ \
    /media/blumh/data/replica/$replicaroom/Sequence_2/
...
ns-train neus-acc \
    ...
    --vis wandb \
    sdfstudio-data \
    --data /media/blumh/data/replica/$replicaroom/Sequence_1/sdfstudio \
    --include-sensor-depth True \
    --include-semantics True

train_id=$(ls -tl /home/blumh/CVG/scan_netter/ScanNetter/outputs/-media-blumh-data-replica-$replicaroom-Sequence_1-sdfstudio/neus-acc/ | head -n 2 | tail -n 1 | awk '{print $9}')
echo $train_id

config=/home/blumh/CVG/scan_netter/ScanNetter/outputs/-media-blumh-data-replica-$replicaroom-Sequence_1-sdfstudio/neus-acc/$train_id/config.yml
# the job below may OOM sometimes, so we wait such that all GPU memory is free
sleep 60

ns-render --camera-path-filename /media/blumh/data/replica/$replicaroom/Sequence_1/sdfstudio/camera_path.json \
    --traj filename \
    --output-format images \
    --rendered-output-names semantics \
    --output-path /media/blumh/data/replica/$replicaroom/Sequence_1/pred_sdfstudio_$train_id.png \
    --load-config $config
