...
def process_image(
  ...
  def combine_bmasks(bmasks, order='descending'):
    """
  ...
  #   instance_id = np.zeros(shape=(H, W), dtype=np.int64)
...
def run(
  ...
  for file in tqdm(input_files):
    ...
    cv2.imwrite(str(output_dir / f'{file.stem}.png'),result.cpu().numpy())
...
def arg_parser():
  ...
  parser.add_argument(
      '--output',
      type=str,
      default='intermediate/instance_groundedsam',
      help=
