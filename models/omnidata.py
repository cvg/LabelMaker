...
def omnidepth_completion(args,
    ...
    def depth_completion(k):
        ...
        omnidepth = cv2.imread(str(scene_dir / args.output / f'{k}.png'), cv2.IMREAD_UNCHANGED)
        ...
        cv2.imwrite(str(scene_dir / args.output / f'{k}.png'),
                    fused_depth)
...
def run(args,
  ...
  for k in tqdm(keys):
    ...
    cv2.imwrite(str(output_dir / f'{k}.png'),
                omnidepth)
  if completion:
