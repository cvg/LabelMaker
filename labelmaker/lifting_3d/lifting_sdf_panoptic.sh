...
# preprocessing
python "$repo_dir"/labelmaker/lifting_3d/preprocessing_sdf_panoptic.py \
  --sampling $subsample \
  --workspace $WORKSPACE

# # train
method=neus-facto
# experiment_name=neus—0518
# Extract the last part of the WORKSPACE path and prepend "IML"
experiment_name="neus-"$(basename "$WORKSPACE")
...
ns-train ${method} \
  ...
  --pipeline.model.sdf-field.instance-num-layers 4 \
  --pipeline.model.sdf-field.instance-layer-width 512 \
  ...
  --pipeline.model.instance-loss-mult 0.1 \
...
# render class labels
render_dir=${WORKSPACE}/neus_lifted_semantic
mkdir -p $render_dir
...
render_dir=${WORKSPACE}/neus_lifted_instance
mkdir -p $render_dir
ns-render --camera-path-filename $preprocess_data_dir/camera_path.json \
  --traj filename \
  --output-format images \
