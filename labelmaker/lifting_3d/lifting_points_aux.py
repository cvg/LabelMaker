...
def main(
    ...
    maximum_label: int,
    # label_weights_file: Union[str, Path], 
):
  scene_dir = Path(scene_dir)
  label_folder = Path(label_folder)
  output_file = Path(output_file)
  output_mesh = Path(output_mesh)
#   label_weights_image = Path(label_weights_image)
...
#   label_weights_image_path = scene_dir / label_weights_image  # 新增路径
  label_weights_image_path = scene_dir/'intermediate/consensus_aux'
  print(label_weights_image_path)
#   assert label_weights_image_path.exists() and label_weights_image_path.is_file()
  assert label_weights_image_path.exists()
  ...
  for idx, file in tqdm(enumerate(files), total=len(files)):
    ...
    labels = np.asarray(Image.open(str(file)))
     ...
     # load aux lable weights image
    label_weights_image = np.asarray(Image.open(str(label_weights_image_path /
                                      f'{frame_key}_aux.png'))).astype(np.uint8)
    labels_aux = label_weights_image[:,:,0]
    labels_votes = label_weights_image[:,:,1]
    labels_aux_votes = label_weights_image[:,:,2]
    ...
    max_label_aux = np.max(labels_aux)
    if max_label_aux > labels_3d.shape[-1] - 1:
      raise ValueError(
          f'Label {max_label_aux} is not in the label range of {labels_3d.shape[-1]}'
      )
    ...
    if resize_image:
      ...
      labels_votes = cv2.resize(labels_votes, (w, h), interpolation=cv2.INTER_NEAREST)
      labels_aux = cv2.resize(labels_aux, (w, h), interpolation=cv2.INTER_NEAREST)
      labels_aux_votes = cv2.resize(labels_aux_votes, (w, h), interpolation=cv2.INTER_NEAREST)
    else:
      h, w, _ = image.shape
    ...
    votes_valid = labels_votes[yy[valid_mask], xx[valid_mask]]
    labels_aux_2d = labels_aux[yy[valid_mask], xx[valid_mask]]
    votes_valid_aux = labels_aux_votes[yy[valid_mask], xx[valid_mask]]

    labels_3d[valid_mask, labels_2d] += votes_valid
    labels_3d[valid_mask, labels_aux_2d] += votes_valid_aux
...
def arg_parser():
  ...
  parser.add_argument(
      '--output',
      type=str,
      default='labels_aux.txt',
      help='Name of files to save the labels',
  )
  parser.add_argument(
      '--output_mesh',
      type=str,
      default='point_lifted_mesh_aux.ply',
      help='Name of files to save the labels',
  )
...
#         '--label_weights_file',
#         type=str,
#         default='label_weights.npy',
#         help='Path to the label weights file',
#     )
...
if __name__ == '__main__':
  ...
  main(
    ...
    #   label_weights_file=args.label_weights_file,
  )
