...
log = logging.getLogger(' IML data preprocessing ')


@gin.configurable
def IML_preprocessing(
...
def arg_parser():
  parser = argparse.ArgumentParser(
      description=
      'Preprocess a scene in LabelMaker format into a IML dataset')
  parser.add_argument(
      '--workspace',
      type=str,
      required=True,
  ...
  parser.add_argument(
      '--output',
      type=str,
      default='intermediate/IML_preprocessing',
      help=
      'Name of output folder in the workspace directory, to store data used in IML(nerfacto panoptic) training and rendering',
  )
