...
def consensus(k, folders, output_dir, output_aux_dir, min_votes):
  ...
  cv2.imwrite(str(output_aux_dir / f'{k}_aux.png'), aux_data.astype(np.uint8))


# this is needed for parallel execution
def wrapper_consensus(k, input_folders_str, output_dir_str,output_aux_dir_str, min_votes):
  input_folders = [Path(s) for s in input_folders_str]
  output_dir = Path(output_dir_str)
  output_aux_dir = Path(output_aux_dir_str)
  consensus(k, input_folders, output_dir,output_aux_dir, min_votes)
  return 1
...
def run(
  ...
  output_aux_folder = Path(output_aux)

  assert scene_dir.exists() and scene_dir.is_dir()

  output_dir = scene_dir / output_folder
  output_aux_dir = scene_dir / output_aux_folder
  # check if output directory exists
  shutil.rmtree(output_dir, ignore_errors=True)
  os.makedirs(str(output_dir), exist_ok=False)
  shutil.rmtree(output_aux_dir, ignore_errors=True)
  os.makedirs(str(output_aux_dir), exist_ok=False)
  ...
  output_aux_dir_str = str(output_aux_dir)
  

  # Using Parallel to run the function in parallel
  results = Parallel(n_jobs=n_jobs)(delayed(wrapper_consensus)(
      k, input_folders_str, output_dir_str,output_aux_dir_str, min_votes) for k in tqdm(keys))
