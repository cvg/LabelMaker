...
scene=scene0458_00
experiment_name=${scene}_semantic_nerf_class_weights
echo $scene
echo $scene

mkdir $TMPDIR/$scene
for SUBDIR in pred_consensus_5_scannet color depth label-filt intrinsic omnidata_depth omnidata_normal pose pred_sam pred_consensus 
do
    echo Copying $SUBDIR ...
	cp -r /cluster/project/cvg/blumh/scannet/$scene/$SUBDIR $TMPDIR/$scene/
done

python scripts/sdfstudio_scannet_preprocessing.py --label_template label-filt --sampling 2 --size 416 --scannetpose \
    $TMPDIR/$scene
